  ...
  div
    .vui-scrollable--x.vui-m-bottom--large
      table.vui-table.vui-table--striped
        thead
          tr
            th(style='width: 80px') Stations
            th(style='width: 100px'): vui-sorting-column(title='Station<br>Order #')
            th(style='width: 130px') Manage #[br] Schedule
            th(style='width: 90px'): vui-sorting-column(title='Agency<br>Order #')
            th(style='width: 110px'): vui-sorting-column(title='Videa<br>Order #')
            th(style='width: 280px'): vui-sorting-column(title='Advertiser')
            th(style='width: 250px'): vui-sorting-column(title='Agency')
            th(style='width: 95px'): vui-sorting-column(title='Estimate')
            th(style='width: 100px'): vui-sorting-column(title='Start Date')
            th(style='width: 100px'): vui-sorting-column(title='End Date')
            th(style='width: 100px'): vui-sorting-column(title='Order<br>Value')
            th(style='width: 100px'): vui-sorting-column(title='Spots<br>Ordered')
            th(style='width: 100px'): vui-sorting-column(title='Open<br>Preempts')
            th(style='width: 100px'): vui-sorting-column(title='Missed<br>Value')
            th(style='width: 100px'): vui-sorting-column(title='Traffic Spots')
            th(style='width: 90px') Order #[br] Completed
            th(style='width: 150px') Date/Time Order #[br] Completed
            th(style='width: 120px') Order #[br] Completed By
        tbody
          tr(
...
<script>
  export default {
    ...
    // },
    computed: {
      hasChangedCompletionOrders () {
        return this.orders.some((order) => order.isVideaArchived)
      }
    },
    methods: {
      complete () {
        this.$toasted.success('Saving...', {
          theme: "primary",
          position: "top-right",
          duration : 3000
        })
