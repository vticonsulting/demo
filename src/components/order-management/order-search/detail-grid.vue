...
.vui-scrollable--x
  table.vui-table.vui-no-row-hover.vui-table--fixed-layout
    thead
      tr
        th(style='width: 80px')
        th(style='width: 78px'): vui-sorting-column(title='Line #')
        th(style='width: 82px') Days
        th(style='width: 110px') Time
        th(style='width: 200px'): vui-sorting-column(title='Program')
        ...
        th.spot-allocation-column(
          ...
          spot-allocation-header(v-model='orderLineItems[0].weeklySpotAllocations')
        th(style='width: 100px') Spot #[br] Rate
        th(v-if='!isImpressionsBuyType' style='width: 100px') Buyer #[br] RTG
        th(v-if='!isImpressionsBuyType' style='width: 100px') Videa #[br] RTG
    ...
    tbody(v-for = '(lineItem, index) in orderLineItems', :class='(index % 2 === 0) ? "vui-highlight" : ""')
      tr(v-bind:class = '{ "vui-is-selected": lineItem.isExpanded }')
        ...
        td(:title='lineItem.days' style='width: 82px') {{lineItem.days}}
        td.vui-truncate(:title='lineItem.time' style='width: 110px') {{lineItem.time}}
        td.vui-truncate(:title='lineItem.programName' style='width: 200px') {{lineItem.program}}
        td(:title='lineItem.spotLength' style='width: 88px') {{lineItem.length}}
                ...
                tbody
                  tr.with-row-span(v-for='trafficLog in lineItem.trafficLogs')
                    ...
                    td.spot-allocation-cell(v-if='isShowSpotAllocations')
                      spot-allocation-with-missed-spots(
                        v-if = 'isMissedSpotsShown'
                        v-bind:spots = 'trafficLog.weeklySpotAllocations || defaultWeeklySpotAllocations'
                        navigator = 'spotAllocationNavigator'
                      )
                      spot-allocation-with-traffic-spots(
                        v-if = '!isMissedSpotsShown'
...
<script>
  export default {
    ...
    methods: {
      ...
      showCurrentInfo(lineItem) {
        this.orderLine = lineItem
