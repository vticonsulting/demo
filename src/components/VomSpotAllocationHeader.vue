<!-- <vom-spot-allocation-header ng-model="items[0].spotAllocations" navigator="ctrl.spotAllocationNavigator" class="ng-pristine ng-untouched ng-valid ng-isolate-scope ng-not-empty"> -->
<template lang="pug">
  .spot-allocation-row
    .spot-allocation-nav-section.pointer
      i.button-icon.left-icon(@click='scrollLeft')
    .spot-allocation-container(:style="{ 'width': spotsWidth }")
      ul.spot-allocation(style='left: 0')
        li(v-for='spot in spots')
          div
            span(:title='spot.week') {{ spot.week }}
    .spot-allocation-nav-section.pointer
      i.button-icon.right-icon(@click='scrollRight')
</template>
