const moment = require('moment')

module.exports = {
  context: {
    campaigns: [
      {
        stationOrderNumber: 876542,
        videaOrderNumber: 131542,
        advertiser: 'Metro Wireless',
        agency: 'The Media Masters Group',
        product: 'GEN',
        estimateNumber: 23,
        revenue: 41000,
        flightStartDate: moment().day(1).format('MM/DD/YY'),
        flightEndDate: moment().add(3, 'weeks').weekday(4).format('MM/DD/YY'),
        campaignDeliveryPercent: '80%',
        currentGrpsImps: 25.3,
        purchasedGrpsImps: 31.5,
        difference: -6.2,
        currentHouseholdGrpsImps: 47.6,
        demoName: 'A18-49',
        orderLastUpdated: moment().subtract(2, 'weeks'),
        weeksRemaining: 3,
        daypartSummary: {
          total: {
            purchasedGrpsImps: 78.8,
            purchasedToDate: 31.5,
            deliveredToDate: 25.3,
            percentage: 0.8,
            difference: -6.2
          },
          items: [
            {
              dayparts: 'EM',
              purchasedToDate: 12.4,
              deliveredToDate: 9.5,
              percentage: 0.77,
              difference: -2.9
            },
            {
              dayparts: 'EN',
              purchasedToDate: 5.8,
              deliveredToDate: 6,
              percentage: 1.03,
              difference: 0.2
            },
            {
              dayparts: 'PR',
              purchasedToDate: 9,
              deliveredToDate: 7.7,
              percentage: 0.86,
              difference: -1.3
            },
            {
              dayparts: 'LN',
              purchasedToDate: 0,
              deliveredToDate: 0,
              percentage: 0,
              difference: 0
            },
            {
              dayparts: 'LF',
              purchasedToDate: 4.3,
              deliveredToDate: 2.1,
              percentage: 0.49,
              difference: -2.2
            },
            {
              dayparts: 'Total',
              purchasedToDate: 31.5,
              deliveredToDate: 25.3,
              percentage: 0.8,
              difference: -6.2
            }
          ]
        }
      },
      {
        stationOrderNumber: 876498,
        videaOrderNumber: 135001,
        advertiser: 'Piedmont Healthcare',
        agency: 'Hughes-Martin Agency',
        product: 'HEA',
        estimateNumber: 57,
        revenue: 37189,
        flightStartDate: 'NOT STARTED',
        flightEndDate: 'NOT STARTED',
        campaignDeliveryPercent: 'N/A',
        currentGrpsImps: 0,
        purchasedGrpsImps: 0,
        difference: null,
        currentHouseholdGrpsImps: 0,
        demoName: 'A25-54',
        orderLastUpdated: '1 week earlier than today\'s date',
        weeksRemaining: 6,
        daypartSummary: {
          total: {
            purchasedGrpsImps: 78.8,
            purchasedToDate: 31.5,
            deliveredToDate: 25.3,
            percentage: 0.8,
            difference: -6.2
          },
          items: [
            {
              dayparts: 'EM',
              purchasedToDate: 12.4,
              deliveredToDate: 9.5,
              percentage: 0.77,
              difference: -2.9
            },
            {
              dayparts: 'EN',
              purchasedToDate: 5.8,
              deliveredToDate: 6,
              percentage: 1.03,
              difference: 0.2
            },
            {
              dayparts: 'PR',
              purchasedToDate: 9,
              deliveredToDate: 7.7,
              percentage: 0.86,
              difference: -1.3
            },
            {
              dayparts: 'LN',
              purchasedToDate: 0,
              deliveredToDate: 0,
              percentage: 0,
              difference: 0
            },
            {
              dayparts: 'LF',
              purchasedToDate: 4.3,
              deliveredToDate: 2.1,
              percentage: 0.49,
              difference: -2.2
            },
            {
              dayparts: 'Total',
              purchasedToDate: 31.5,
              deliveredToDate: 25.3,
              percentage: 0.8,
              difference: -6.2
            }
          ]
        }
      },
      {
        stationOrderNumber: 876599,
        videaOrderNumber: 133529,
        advertiser: 'Papa Johns Pizza',
        agency: 'Zone Group Advertising',
        product: 'HOL',
        estimateNumber: 8,
        revenue: 10650,
        flightStartDate: 'ENDED',
        flightEndDate: 'ENDED',
        campaignDeliveryPercent: '104%',
        currentGrpsImps: 36,
        purchasedGrpsImps: 34.5,
        difference: 1.5,
        currentHouseholdGrpsImps: 37.9,
        demoName: 'A18-49',
        orderLastUpdated: '2 months earlier than today\'s date',
        weeksRemaining: 0,
        daypartSummary: {
          total: {
            purchasedGrpsImps: 78.8,
            purchasedToDate: 31.5,
            deliveredToDate: 25.3,
            percentage: 0.8,
            difference: -6.2
          },
          items: [
            {
              dayparts: 'EM',
              purchasedToDate: 12.4,
              deliveredToDate: 9.5,
              percentage: 0.77,
              difference: -2.9
            },
            {
              dayparts: 'EN',
              purchasedToDate: 5.8,
              deliveredToDate: 6,
              percentage: 1.03,
              difference: 0.2
            },
            {
              dayparts: 'PR',
              purchasedToDate: 9,
              deliveredToDate: 7.7,
              percentage: 0.86,
              difference: -1.3
            },
            {
              dayparts: 'LN',
              purchasedToDate: 0,
              deliveredToDate: 0,
              percentage: 0,
              difference: 0
            },
            {
              dayparts: 'LF',
              purchasedToDate: 4.3,
              deliveredToDate: 2.1,
              percentage: 0.49,
              difference: -2.2
            },
            {
              dayparts: 'Total',
              purchasedToDate: 31.5,
              deliveredToDate: 25.3,
              percentage: 0.8,
              difference: -6.2
            }
          ]
        }
      },
      {
        stationOrderNumber: 875912,
        videaOrderNumber: 131642,
        advertiser: 'Southeast Ford',
        agency: 'Auto Advertising Agency',
        product: 'END',
        estimateNumber: 22,
        revenue: 41450,
        flightStartDate: 'ENDED',
        flightEndDate: 'ENDED',
        campaignDeliveryPercent: '91%',
        currentGrpsImps: 87,
        purchasedGrpsImps: 96,
        difference: -9,
        currentHouseholdGrpsImps: 69.1,
        demoName: 'A25-54',
        orderLastUpdated: '1 month\'s earlier than today\'s date',
        weeksRemaining: 0,
        daypartSummary: {
          total: {
            purchasedGrpsImps: 78.8,
            purchasedToDate: 31.5,
            deliveredToDate: 25.3,
            percentage: 0.8,
            difference: -6.2
          },
          items: [
            {
              dayparts: 'EM',
              purchasedToDate: 12.4,
              deliveredToDate: 9.5,
              percentage: 0.77,
              difference: -2.9
            },
            {
              dayparts: 'EN',
              purchasedToDate: 5.8,
              deliveredToDate: 6,
              percentage: 1.03,
              difference: 0.2
            },
            {
              dayparts: 'PR',
              purchasedToDate: 9,
              deliveredToDate: 7.7,
              percentage: 0.86,
              difference: -1.3
            },
            {
              dayparts: 'LN',
              purchasedToDate: 0,
              deliveredToDate: 0,
              percentage: 0,
              difference: 0
            },
            {
              dayparts: 'LF',
              purchasedToDate: 4.3,
              deliveredToDate: 2.1,
              percentage: 0.49,
              difference: -2.2
            },
            {
              dayparts: 'Total',
              purchasedToDate: 31.5,
              deliveredToDate: 25.3,
              percentage: 0.8,
              difference: -6.2
            }
          ]

        }
      },
      {
        stationOrderNumber: 875888,
        videaOrderNumber: 131634,
        advertiser: 'Community Loans Inc.',
        agency: 'Vanthrop and Associates',
        product: 'QTR',
        estimateNumber: 3567,
        revenue: 19100,
        flightStartDate: moment().subtract(3, 'weeks').weekday(1).format('MM/DD/YY'),
        flightEndDate: moment().add(1, 'weeks').weekday(7).format('MM/DD/YY'),
        campaignDeliveryPercent: '96%',
        currentGrpsImps: 51.9,
        purchasedGrpsImps: 54,
        difference: -2.1,
        currentHouseholdGrpsImps: 103.8,
        demoName: 'A25-54',
        orderLastUpdated: '3 weeks earlier than today\'s date',
        weeksRemaining: 1,
        daypartSummary: {
          total: {
            purchasedGrpsImps: 78.8,
            purchasedToDate: 31.5,
            deliveredToDate: 25.3,
            percentage: 0.8,
            difference: -6.2
          },
          items: [
            {
              dayparts: 'EM',
              purchasedToDate: 12.4,
              deliveredToDate: 9.5,
              percentage: 0.77,
              difference: -2.9
            },
            {
              dayparts: 'EN',
              purchasedToDate: 5.8,
              deliveredToDate: 6,
              percentage: 1.03,
              difference: 0.2
            },
            {
              dayparts: 'PR',
              purchasedToDate: 9,
              deliveredToDate: 7.7,
              percentage: 0.86,
              difference: -1.3
            },
            {
              dayparts: 'LN',
              purchasedToDate: 0,
              deliveredToDate: 0,
              percentage: 0,
              difference: 0
            },
            {
              dayparts: 'LF',
              purchasedToDate: 4.3,
              deliveredToDate: 2.1,
              percentage: 0.49,
              difference: -2.2
            },
            {
              dayparts: 'Total',
              purchasedToDate: 31.5,
              deliveredToDate: 25.3,
              percentage: 0.8,
              difference: -6.2
            }
          ]
        }
      },
      {
        stationOrderNumber: 871211,
        videaOrderNumber: 134787,
        advertiser: 'Sun Valley Bank',
        agency: 'Mullberry Way Group',
        product: 'GEN',
        estimateNumber: 459,
        revenue: 52215,
        flightStartDate: 'NOT STARTED',
        flightEndDate: 'NOT STARTED',
        campaignDeliveryPercent: 'N/A',
        currentGrpsImps: 0,
        purchasedGrpsImps: 0,
        difference: null,
        currentHouseholdGrpsImps: null,
        demoName: 'A25-54',
        orderLastUpdated: '1 month earlier than today\'s date',
        weeksRemaining: 8,
        daypartSummary: {
          total: {
            purchasedGrpsImps: 78.8,
            purchasedToDate: 31.5,
            deliveredToDate: 25.3,
            percentage: 0.8,
            difference: -6.2
          },
          items: [
            {
              dayparts: 'EM',
              purchasedToDate: 12.4,
              deliveredToDate: 9.5,
              percentage: 0.77,
              difference: -2.9
            },
            {
              dayparts: 'EN',
              purchasedToDate: 5.8,
              deliveredToDate: 6,
              percentage: 1.03,
              difference: 0.2
            },
            {
              dayparts: 'PR',
              purchasedToDate: 9,
              deliveredToDate: 7.7,
              percentage: 0.86,
              difference: -1.3
            },
            {
              dayparts: 'LN',
              purchasedToDate: 0,
              deliveredToDate: 0,
              percentage: 0,
              difference: 0
            },
            {
              dayparts: 'LF',
              purchasedToDate: 4.3,
              deliveredToDate: 2.1,
              percentage: 0.49,
              difference: -2.2
            },
            {
              dayparts: 'Total',
              purchasedToDate: 31.5,
              deliveredToDate: 25.3,
              percentage: 0.80,
              difference: -6.2
            }
          ]
        }
      }
    ]
  }
}
